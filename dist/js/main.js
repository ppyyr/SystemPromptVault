...
const isLikelyPathString = (value) =>
  typeof value === "string" && (value.includes("/") || value.includes("\\") || value.startsWith("~"));

const normalizeChangedPaths = (input) => {
  if (Array.isArray(input)) {
    return input.filter((item) => typeof item === "string" && item.trim()).map((item) => item.trim());
  }
...
const handleConfigFileChanged = async (changedPaths = null) => {
  ...
  console.log(
    `[FileChange] Config file changed, editorDirty: ${state.editorDirty}, files: ${
      normalizedPaths.length ? normalizedPaths.join(", ") : "unknown"
    }`
  );
  dismissFileChangeToast();
  if (state.editorDirty) {
    ...
    const toastMessage = await buildConfigUpdatedToastMessage(normalizedPaths);
      ...
      state.fileChangeUnlisten = await listen("config-file-changed", async (event) => {
        ...
        try {
          ...
          const fallbackClientId =
            !payloadIsObject && typeof payload === "string" && !isLikelyPathString(payload) ? payload : null;
          const eventClientId = payloadIsObject
            ? payload.client_id ?? payload.clientId ?? payload.client ?? fallbackClientId
            : fallbackClientId;
          const rawPaths = payloadIsObject
            ? payload.paths ?? payload.path ?? payload.file_path ?? payload.filePath ?? null
            : Array.isArray(payload)
              ? payload
              : isLikelyPathString(payload)
                ? payload
                : null;
          const normalizedPaths = normalizeChangedPaths(rawPaths);
