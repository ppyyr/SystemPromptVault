...
/**
 * 格式化路径显示: 将绝对路径转换为 ~ 格式
 * @param {string} fullPath - 完整的文件路径
 * @param {string} userHomeDir - 用户主目录路径
...
export const formatFilePathsForToast = (filePaths, userHomeDir) => {
  ...
  const prefix = finalPaths.slice(0, 2).join(", ");
  const language = getCurrentLanguage();
  if (language === "zh") {
    return `${prefix} 等${finalPaths.length}个文件`;
  }
  return `${prefix} and ${finalPaths.length} files in total`;
};
